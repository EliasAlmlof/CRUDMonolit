...
public interface UserRepo extends JpaRepository<User, Long> {
    ...
    @Query(value = "SELECT COUNT(*) > 0 FROM festival_user WHERE username = :val" , nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM festival f JOIN booking b ON f.festival_id = b.festivalid " +
            "WHERE b.userid = :userid ORDER BY f.festival_date, f.festival_name", nativeQuery = true)
    ...
    @Modifying
    @Query(value = "UPDATE festival_user SET email = :val1 WHERE user_id = :val2",nativeQuery = true)
    void updateEmail(@Param("val1") String email, @Param("val2") int id);

    @Modifying
    @Query(value = "DELETE FROM festival_user WHERE user_id = :val", nativeQuery = true)
