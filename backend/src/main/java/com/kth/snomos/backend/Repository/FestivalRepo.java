...
public interface FestivalRepo extends JpaRepository<Festival, Long> {
    ...
    @Query(value = "SELECT * FROM festival WHERE LOWER(festival_name) LIKE LOWER(CONCAT('%', :name, '%')) ORDER BY festival_date, festival_name", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM festival WHERE LOWER(festival_location) LIKE LOWER(CONCAT('%', :location, '%')) ORDER BY festival_date, festival_name", nativeQuery = true)
    ...
    @Query(value =  "SELECT DISTINCT f.* FROM festival f " +
                    ...
                    "WHERE LOWER(a.artist_name) LIKE LOWER(CONCAT('%', :artist, '%'))" +
